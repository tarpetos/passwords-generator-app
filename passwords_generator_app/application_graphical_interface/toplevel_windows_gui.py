...
from ..user_actions_processing.default_alphabet import (
    DEFAULT_LETTERS,
    DEFAULT_DIGITS,
    DEFAULT_PUNCTUATION,
...
def generator_history_screen(lang_state: str):
    ...
    info_label = CTkLabel(
        history_frame,
        text=LOCALIZATION_DATA[lang_state]["toplevel_windows"]["history_window_data"],
    )
...
def password_strength_screen(lang_state: str):
    ...
    enter_label = CTkLabel(
        strength_frame,
        text=LOCALIZATION_DATA[lang_state]["toplevel_windows"]["strength_window_data"][
            "enter_label"
        ],
    )
    ...
    strength_labels = (
        shanon_score_value_label,
        ai_score_value_label,
        average_score_value_label,
        reliability_value_label,
    )
    ...
    str_var_modifier.trace(
        "w", lambda name, index, mode, sv=str_var_modifier: entry_modified(sv)
    )
    password_input_entry = CTkEntry(
        strength_frame, textvariable=str_var_modifier, width=300
    )
    ...
    strength_bar = CTkProgressBar(
        strength_frame, height=20, progress_color=default_progress_bar_color
    )
    strength_bar.set(0)
    ...
    set_equal_grid_segments_size(
        strength_frame, strength_frame_column_number, strength_frame_row_number
    )
    ...
    def update_rectangle(rating: float):
        ...
        strength_bar.configure(
            progress_color=default_progress_bar_color if rating == 0 else color
        )
...
def search_screen(
    lang_state: str, search_query: str, data_list: Dict[str, List[Tuple[int | str]]]
):
    ...
    label = CTkLabel(
        top_frame,
        text=LOCALIZATION_DATA[lang_state]["toplevel_windows"]["search_window_data"],
    )
...
def database_search(event: Any, lang_state: str) -> None:
    ...
    data_list = retrieve_data_for_build_table_interface(
        lang_state, column_number=3, user_query=user_search
    )
    print(data_list)
...
def alphabet_screen(lang_state: str):
    ...
    letters_label = CTkLabel(
        alphabet_window,
        text=LOCALIZATION_DATA[lang_state]["toplevel_windows"]["alphabet_window_data"][
            "labels"
        ]["letters_label"],
    )
    ...
    set_equal_grid_segments_size(
        alphabet_window, alphabet_window_column_number, alphabet_window_row_number
    )
...
def clear_entries(
    letter_entry: CTkEntry, digits_entry: CTkEntry, punctuation_entry: CTkEntry
):
...
def check_for_punctuation(user_input: str) -> bool:
    ...
    for char in user_input:
        if (
            char in NOT_ALLOWED_PUNCTUATION
            or char.isdigit()
            or char.isalpha()
            or char.isspace()
        ):
...
def save_custom_alphabet(
    ...
    if (
        not check_for_repeated_characters(new_letters)
        or not check_for_repeated_characters(new_digits)
        or not check_for_repeated_characters(new_punctuation)
    ):
    ...
    if (alphabet_data is None) and (
        new_letters == DEFAULT_LETTERS
        and new_digits == DEFAULT_DIGITS
        and new_punctuation == DEFAULT_PUNCTUATION
    ):
        print("Fields are not changed! Its default values.")
    elif (alphabet_data is not None) and (
        new_letters == alphabet_data[1]
        and new_digits == alphabet_data[2]
        and new_punctuation == alphabet_data[3]
    ):
...
def back_to_default_alphabet(
    ...
    if (alphabet_data is None) or (
        (alphabet_data is not None)
        and (
            alphabet_data[1] == DEFAULT_LETTERS
            and alphabet_data[2] == DEFAULT_DIGITS
            and alphabet_data[3] == DEFAULT_PUNCTUATION
        )
    ):
        print("Already is default!")
    else:
        db_connector.update_alphabet(
            DEFAULT_LETTERS, DEFAULT_DIGITS, DEFAULT_PUNCTUATION
        )
...
def set_equal_grid_segments_size(
    frame: CTkFrame | CTkToplevel, column_number: int, row_number: int
):
