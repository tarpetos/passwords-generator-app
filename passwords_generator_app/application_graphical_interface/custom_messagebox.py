...
class BaseMessageBox(CTkToplevel):
    def __init__(self, *args, **kwargs):
        ...
        )  # ignore warning about using not CTkImage
    ...
    def inner_frame_content(
        self, title: str | None, message: str | None, image: str, wrap_length: int
    ):
        self.title(title)
    ...
    def base_show(
        ...
        close_button = CTkButton(
            self._messagebox_frame, text=button_option, command=lambda: self.destroy()
        )
    ...
    def base_ask(
        ...
        true_button = CTkButton(
            self._messagebox_frame,
            text=button_options[0],
            command=lambda: response.set(True),
        )
        true_button.pack(side="left", fill="both", expand=True, padx=20, pady=5)

        false_button = CTkButton(
            self._messagebox_frame,
            text=button_options[1],
            command=lambda: response.set(False),
        )
...
def show_info(
    title: str | None,
    message: str | None,
    button_option: str = "Ok",
    wrap_length: int = 0,
) -> None:
    message_box = BaseMessageBox()
    message_box.base_show(
        title, message, message_box.info_image, wrap_length, button_option
    )
...
def show_error(
    ...
    message_box = BaseMessageBox()
    message_box.base_show(
        title, message, message_box.error_image, wrap_length, button_option
    )
...
def show_warning(
    ...
    message_box = BaseMessageBox()
    message_box.base_show(
        title, message, message_box.warning_image, wrap_length, button_option
    )
