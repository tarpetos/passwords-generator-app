...
def get_search_data_from_database_table(all_queries) -> list:
    full_list_of_data = [
        database_user_data.select_search_data_by_desc(record) for record in all_queries
    ]

    en_tuple = english_tuple_of_columns_names[:3]
    uk_tuple = ukrain_tuple_of_columns_names[:3]
    full_list_of_data = check_columns_names(full_list_of_data, en_tuple, uk_tuple)

    return full_list_of_data


def check_columns_names(full_list_of_data,
                        en_tuple=english_tuple_of_columns_names,
                        uk_tuple=ukrain_tuple_of_columns_names) -> list:
    global lang_state
    if lang_state:
        full_list_of_data.insert(0, en_tuple)
        lang_state = True
    else:
        full_list_of_data.insert(0, uk_tuple)
        lang_state = False
                        ...
                        while True:
                            ...
                            if save_pass == '' or save_pass:
                                ...
                                if re.match(local_choice_pattern, save_pass):
                                    load_screen = app_loading_screen(application_window, lang_state)
...
def result_of_connection(application_window) -> RemoteDB | None:
    ...
    if not check_internet_connection():
        load_screen.destroy()
        connection_error_message(lang_state)
        return
...
def update_columns_via_app_interface(all_data_from_table):
...
def control_mysql_connection() -> RemoteDB | int:
    try:
        ...
        connection_timeout_message(lang_state)
        return -1
    except mysql.connector.errors.DatabaseError:
        error_sync_message(lang_state)
        return -1
...
def database_search(event):
    while True:
        user_search = search_query_input_message(lang_state)

        if user_search is None:
            return
        elif user_search == '' or len(user_search) > HALF_VARCHAR:
